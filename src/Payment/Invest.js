...
export default class Invest extends Component {
    ...
    number_Validation = (v) => {
        ...
        if (number.test(v)) {
            //     console.log('in',v)
            if (v<=this.props.TotalCompanyAmount) {
                ...
                let temp = (v / 100) * GivenEquity;
                let Ans = (temp / TotalCompanyAmount) * 100
                this.setState({ equity: Ans.toFixed(2) })
            }else{
                this.setState({
                    Amount: '',
                    validate1: false
                })
                alert('Amount should be less than Company Investment Require')
            }
        }
        else if (v.length == 0) {
            this.setState({
                    ...
                    <View style={style.main_view}>
                        ...
                        <View style={style.main2_view}>
                           <View style={{flexDirection:'row'}}>
                               <Text style={{color:'#878787',fontSize:18,alignSelf:'center'}}>£</Text>
                           <TextInput allowFontScaling={false} value={this.state.Amount} onChangeText={(value) => this.number_Validation(value)} style={[style.input_style, !this.state.validate1 ? style.error : null]} placeholder="7000" placeholderTextColor="#878787" ></TextInput>
                           </View>
                            <View style={style.v1}>
                            ...
                            <View style={style.equity}>
                                ...
                                <Text allowFontScaling={false} style={style.text2}>{this.state.equity}%</Text>
                            </View>
                        </View>
                    </View>
                </Content>
...
const style = StyleSheet.create({
    ...
    input_style: { height: 50, width: '100%', borderBottomColor: '#EC9705', borderBottomWidth: 1, color: "#878787", fontSize: 16 },
