...
export default class InvestmentDetail extends Component {
    ...
    componentDidMount = () => {
        ...
        //console.log(temp)
        this.setState({ MileStone: temp, MileStoneData: temp2, tempdate: temp3 })
    }
    selectDropdown = (i) => {
        let flag = true;
        console.log("data=>",this.state.MileStoneData)
        for (let index = 0; index < i; index++) {
            if (this.state.MileStoneData[index].project_tittle == '' | this.state.MileStoneData[index].project_description == '' | this.state.MileStoneData[index].milestone_start == null | this.state.MileStoneData[index].milestone_end == null | this.state.MileStoneData[index].amount <= 0 | this.state.MileStoneData[index].filename == 'Attache your document here') {
                console.log('false ',this.state.MileStoneData[index].project_tittle,this.state.MileStoneData[index].project_description,index,this.state.MileStoneData[index].milestone_start ,this.state.MileStoneData[index].milestone_end ,this.state.MileStoneData[index].filename)
                flag = false;
                break;
            }else {
                flag=true
        ...
        if (i == 0) {
            ...
            alert('Fill the above milestose completely')
        }
    }
    ...
    SetStartDate = (index, v) => {
        ...
        //  console.log(this.state.ke)
        let temp = this.state.MileStoneData;
        let temp2 = this.state.tempdate
        temp2[index].milestone_start = v
        temp[index].milestone_start = vv;
        this.setState({ MileStoneData: temp, tempdate: temp2 })
        this.props.SendData(3, this.state.MileStoneData)

    }
    SetEndDate = (index, v) => {
        // console.log("Date=>",v.getDate())
        let vv = moment(v).format('YYYY-MM-DD') + " " + moment().format().slice(11, 19);
        let temp = this.state.MileStoneData;
        let temp2 = this.state.tempdate
        temp[index].milestone_end = vv;
        temp2[index].milestone_end = v
        this.setState({ MileStoneData: temp, tempdate: temp2 })
        this.props.SendData(3, this.state.MileStoneData)
    }
    ...
    SetAmount = (index, v) => {
        ...
        if (number.test(v)) {
            ...
            let MEquity=this.state.tempdate
            let GivenEquity=this.props.Equity;
            let TotalCompanyAmount=this.props.TotalCompanyAmount;
            let Etemp=(v/100)*GivenEquity;
            let Ans=(Etemp/TotalCompanyAmount)*100;
            MEquity[index].Equity=Ans.toFixed(2)
            //console.log(this.state.tempdate)
            this.setState({tempdate:MEquity})
                                            ...
                                            <View style={styles.v11}>
                                                ...
                                                <View style={styles.v22}>
                                                    <DatePicker
                                                        ...
                                                        minimumDate={key > 0 ? this.state.tempdate[key - 1].milestone_end == null ? new Date() : new Date(this.state.tempdate[key - 1].milestone_end) : new Date()}
                                                        ...
                                                        animationType={"fade"}
                                                        androidMode={"default"}
                                                        placeHolderText={data.milestone_start == null ? 'Start Date' : data.milestone_start.slice(0, 10)}
                                                ...
                                                <View style={styles.v22}>
                                                    <DatePicker
                                                        placeHolderTextStyle={{ color: "#878787" }}
                                                        // defaultDate={new Date(2018, 4, 4)}
                                                        minimumDate={this.state.tempdate[key].milestone_start == null ? new Date() : new Date(this.state.tempdate[key].milestone_start)}
                                                        //maximumDate={new Date(2018, 12, 31)}
                                                        locale={"en"}
                                                        hideText={true}
                                                        timeZoneOffsetInMinutes={undefined}
                                                        ...
                                                        animationType={"fade"}
                                                        androidMode={"default"}
                                                        placeHolderText={data.milestone_end == null ? 'End Date' : data.milestone_end.slice(0, 10)}
                                                        textStyle={{ color: "#878787" }}
                                                        onDateChange={(val) => { this.SetEndDate(key, val), this.setState({ sdate: val }) }}
                                                        // onDateChange={date => this.setDate(date)}
                                                        disabled={false}
                                                    />
                                                    <Icon name="calendar" style={{ color: '#EC9705' }} type="AntDesign" />
                                                ...
                                                <View style={styles.v33}>
                                                    ...
                                                    <Text allowFontScaling={false} style={styles.text22}>{this.state.tempdate[key].Equity}%</Text>
                                                </View>
                                            </View>
