...
export default class LogIn extends Component {
  ...
  //        console.log('Email at log in,',resjson.Email)
        ...
        await fetch(this.state.url + '/LogIn/LogIn', {
          ...
          .then(resjson => {
            if (resjson.Successful) {
              ...
              console.log('hy');
              Actions.select({user_id:resjson.data[0].id})
 }
            else {
              this.setState({ loading: false })
              console.log('failed');
        ...
        <ImageBackground resizeMode="stretch" source={require('../images/signup.png')} style={styles.bgimg}>
          <ScrollView >
            <View style={styles.vmargin}>
              <Text allowFontScaling={false} style={[styles.ltext, { marginLeft: 10 }]}>LOGIN </Text>
