...
export default class LogIn extends Component {
    ...
    if (name.test(v)) {
      if (this.state.k == key) {
        ...
        this.state.data.push({
          answer: this.state.temp,
          q_id: this.state.q_id,
          question: this.state.question,
          category: this.state.category,
          type: this.state.type,
          cat_id: this.state.c_id,
        });
          ...
          for (var i = 0; i < this.state._data.length; i++) {
            ...
            for (var j = 0; j < this.state._data[i].e_questions.length; j++) {
              this.state._ans_data[i].push({
                ...
                question: this.state._data[i].e_questions[j]
  ...
  multi_input_data = async () => {
    await fetch(
      this.state.url + '/E_Question/Entrepreneur_questions_multiple',
      {
        method: 'get',
        headers: {
          Accept: 'application/json',
  ...
  mandatory_input_data = async () => {
    await fetch(
      this.state.url + '/E_Question/Entrepreneur_questions_mandatory',
      {
        method: 'get',
        headers: {
          Accept: 'application/json',
              ...
              {this.state._data.map((val, i) => (
                <View>
                  <Text style={[{position: 'absolute', opacity: 0}]}>
                    {i == 0
                      ? this.state._toggle.push({hide: 0})
                      : this.state._toggle.push({hide: 1})}
                  </Text>
                  <View>
                                ...
                                {q.type == 'text_area' ? (
                                  <View>
                                    <Textarea
                                      ...
                                      value={
                                        this.state._ans_data.length > 0
                                          ? this.state._ans_data[i][j]
                                          : ''
                                      }
                                  ...
                                  <View>
                                    ...
                                    <Label
                                      style={[
                                        styles.username,
                                        styles.itemstyle,
                                        {borderBottomWidth: 0, marginTop: 30},
                                      ]}>
                                      {q.question_description}
                                    </Label>
                                    <Item
                                      ...
                                      <Picker
                                        ...
                                        selectedValue={
                                          this.state._ans2_data.length > 0
                                            ? this.state._ans2_data[i][j]
                                                .answere
                                            : ''
                                        }
                                        onValueChange={value =>
                                    ...
                                    {this.state.sub_array[0].map((val, key) => (
                                      <ListItem style={{borderBottomWidth: 0}}>
                                        ...
                                        <CheckBox
                                          ...
                                          checked={
                                            this.state.check_value[key].status
                                          }
                                        />
                                        <Text style={styles.username}>
                                          {val.label}
