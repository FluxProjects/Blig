...
export default class Register8 extends Component {
  ...
  componentDidMount = () => {
    ...
    if (this.props.Type == 'Investor') {
      this.setState({
        img: Pic6,
        colorvalue: temp,
        ServicesReuired: this.props.Services,
      });
    } else {
      this.setState({
        img: Pic8,
  ...
  Reverse = () => {
    if (this.props.Type == 'Investor') {
      //in case of this file work as Reg6
  ...
  render_service_sector = () => {
    return this.state.service_sector_stacks.map((s, j) => (
          ...
          <View style={[styles.v2style, {marginTop: 10}]}>
            <TouchableWithoutFeedback onPress={() => this.selectedbox(0)}>
              <View
                style={[
                  styles.v3style,
                  {backgroundColor: this.state.colorvalue[0]},
                ]}>
                {/* <Image source={require('../images/promotion.png')} style={styles.imgsize22} /> */}
                <Image
                  source={{
                    uri:
                      'https://flux.pk/blig-web/storage/app/public/upload/' +
                      s.ss_image,
                  }}
                  style={styles.imgsize22}
                />
              </View>
            </TouchableWithoutFeedback>
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <ImageBackground
          resizeMode="stretch"
          source={require('../images/breg8.png')}
          style={styles.imagebackground}>
          <Image
            source={
              this.props.Type == 'Investor'
                ? require('../images/reg6.png')
                : this.props.LoanFinancing > 0
                ? require('../images/reg9.png')
                : this.state.img
            }
            style={styles.image}
          />
          <View style={styles.mainview}>
            <Text allowFontScaling={false} style={styles.text11}>
              Select required services
            </Text>
            {/*
             *todo verify below text with dev team.
             */}
            <Text
              allowFontScaling={false}
              style={[
                styles.text11,
                {color: '#EC9705', marginTop: -7, marginBottom: 15},
              ]}
            />
            <View>{this.render_service_sector()}</View>
            {/* <View style={styles.v1style}>
                                <View style={[styles.v2style, { marginTop: 10 }]}>
                                    <TouchableWithoutFeedback onPress={() => this.selectedbox(0)}>
