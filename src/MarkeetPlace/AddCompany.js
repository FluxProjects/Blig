...
export default class AddCompany extends Component {
    ...
    await fetch(this.state.uri + '/Image/UploadImages', {
      ...
      .then(response => {
        ...
        if (response.Successful) {
          this.setState({ logodata: null, logouri: { uri: this.state.logodata.uri },logo:true });
        }
        else {
          alert("Upload Failed")
        }
  ...
  AddCompany = () => {
    if (this.state.flag) {
      ...
      if ( this.state.companyname == '' | this.state.RegNo == '' | this.state.salesequity <= 0 | this.state.videouri == 'video uri',
